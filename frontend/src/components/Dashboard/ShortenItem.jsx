...
const ShortenItem = ({ originalUrl, shortUrl, clickCount, createdDate }) => {
    ...
    const fetchMyShortUrl = async () => {
        setLoader(true);
        try {
             // Generate dynamic date range - last 30 days
             const today = new Date();
             const thirtyDaysAgo = new Date(today);
             thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
             
             // Format as ISO_LOCAL_DATE_TIME: yyyy-MM-dd'T'HH:mm:ss
             ...
             const startDate = formatDateTime(thirtyDaysAgo);
             const endDate = formatDateTime(today);
             
             console.log("Analytics Request - startDate:", startDate, "endDate:", endDate);
      ...
      <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden ">
        <div className="text-slate-900 pb-1 sm:pb-0   flex items-center gap-2 ">
            {/* <a href={`${import.meta.env.VITE_REACT_SUBDOMAIN}/${shortUrl}`}
                target="_blank"
                className="text-[17px] font-montserrat font-[600] text-primary hover:text-secondary">
                {subDomain + "/" + `${shortUrl}`}
            </a> */}
        ...
        <div className="flex  flex-1  sm:justify-end items-center gap-4">
            ...
            >
                <div className="flex cursor-pointer gap-1 items-center bg-primary hover:bg-secondary py-2 font-semibold shadow-md shadow-slate-500 px-6 rounded-md text-white transition-colors">
            ...
            <div
                ...
                className="flex cursor-pointer gap-1 items-center bg-accent hover:brightness-90 py-2 font-semibold shadow-md shadow-slate-500 px-6 rounded-md text-white transition-all"
            >
